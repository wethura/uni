...
//		lm_lab_listByPageNum_{pageNum}
	...
	@PostMapping
	@ResponseBody
	public Result create(Lab lab, String adminsStr) throws Exception {

		List<String> admins = new ArrayList<String>();
		if (adminsStr != null || adminsStr != ""){
			admins = Arrays.asList(adminsStr.split("、"));
		}
	 ...
	 */
	@ApiOperation(value = "查询所有实验室的id和name", notes = "")
	@GetMapping("listAllEname")
	@ResponseBody
	public Result listAllEName() throws IOException {
		List<Map<String,Object>> list = labService.selectAllEname();
		if (list == null){
			return Result.build(ResultType.Failed);
		}else {
	...
	@PutMapping("/update")
	@ResponseBody
	public void update(HttpServletResponse response, Lab lab, String adminsStr)throws Exception{
	 ...
	 * @author 招黄轩
	 */
	@ApiOperation(value="指定实验室管理员为一个")
	 ...
	 * @return
	 */
	@ApiOperation(value = "指定某实验室的全部管理员")
	@PutMapping("assignAdminMore")
	@ResponseBody
	public Result assginAdmin(Long labId, String adminStr){

		boolean success = labAdminService.updateLabAdmin(adminStr, labId);
		if (success){
			cache.deleteByPaterm(CacheNameHelper.base);
			return Result.build(ResultType.Success);
		}else {
	 ...
	 * @author 招黄轩
	 */
	@ApiOperation(value = "分页高级筛选所有实验室2", notes = "")
	@ApiImplicitParam(name= "json", value = "请求参数", required = true, dataType = "Map")
	@PostMapping("labs/filter/all2")
	public void filterAll2(@RequestBody(required = false) Map<String,Object> json,
	                       HttpServletResponse response) throws IOException {
		response.setContentType("application/json;charset=utf-8");
		PageInfo<Map<String,Object>> pageInfo = labService.filterAll2(json);
		String res = Result.build(ResultType.Success).appendData("pageInfo", pageInfo).convertIntoJSON();
		response.getWriter().write(res);
	}
}
