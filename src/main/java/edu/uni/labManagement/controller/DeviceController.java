...
//		查看实验室中的设备  lm_device_listByLab_{labId}
	...
	public Result destroyByTwo(){
		...
		return Result.build(ResultType.Success).appendData("res", user);
	}

	@ApiOperation(value = "移出设备")
	@DeleteMapping("/{labId}/{deviceId}")
	@ResponseBody
	public Result destory2(@PathVariable Long labId, @PathVariable Long deviceId){

		boolean success = deviceService.deletedLabDevice(labId, deviceId);
		if (success){
			cache.deleteByPaterm(CacheNameHelper.listByLab + "*");
			return Result.build(ResultType.Success);
		}
		return Result.build(ResultType.Failed);
	}

	@ApiOperation(value = "增加单个设备")
	@PostMapping("/addSingle")
	@ResponseBody
	public Result insertSingle(String number,Long deviceModelId, Long labId ){

		LabPojo lab = labService.selectById(labId);
		Long id = inStorageService.deviceModelToDevice(deviceModelId, lab.getDepartmentId(), 0);
		boolean success = deviceService.updateDevice(id, number, labId);
		if (success){
