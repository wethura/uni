...
public class RoutineMaintenanceDetailController {
	...
	public void receive2(HttpServletResponse response, @PathVariable long routineMaintenanceId) throws Exception{
		...
		if (json == null || json == "") {
			...
			List<Map<String, Object>> list = routineMaintenanceDetailService.TurnFromMaintenanceDetail(details);
			System.out.println(details + " " + details.size());
			json = Result.build(ResultType.Success).appendData("res", list).convertIntoJSON();
			if (details != null) {
				cache.set(cacheName, json);
			}
		}
